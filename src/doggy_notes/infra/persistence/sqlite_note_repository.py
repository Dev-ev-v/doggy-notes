...
class SQLiteNoteRepository(NoteRepository):
    ...
    def get_by_tags(
        ...
        logger.debug(
            "Searching notes by tags (include=%s exclude=%s mode=%s) from %s",
            includes,
            excludes,
            mode,
            location
        ...
        where_clauses = [f"deleted_at {condition}"]
        params: list = []
    ...
    def get_by_titles(
        ...
        logger.debug(
            "Searching notes by titles (include=%s exclude=%s) from %s",
            includes,
            excludes,
            location
        )
        ...
        if includes:
            like_clauses = " OR ".join("title LIKE ?" for _ in includes)
            where_clauses.append(f"({like_clauses})")
            params.extend(f"%{t}%" for t in includes)
        ...
        rows = cursor.fetchall()
        logger.debug("%d notes found matching titles", len(rows))

        notes = [NoteMapper.from_row(row) for row in rows]
        for note in notes:
            note.tags = self._load_tags(note.id)
        return notes
