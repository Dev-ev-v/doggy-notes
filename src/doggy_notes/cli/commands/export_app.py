...
def _run_export(*, note_ids=None, tags=None, mode="AND", path=None):
    ...
    try:
        ...
        file = deps.export_notes.execute(result, path)
...
def export_by_id(
    ...
    _run_export(note_ids=note_ids, path=path)
...
def export_by_tag(
    ...
    _run_export(tags=tags, mode=mode, path=path)
